...
面向 TI MSPM0 的桌面工具箱，提供 CCS ↔ Keil 工程转换与独立的 Keil TI 环境配置。桌面端使用 Tauri 2，转换核心使用 Rust，界面不依赖前端框架。
...
- 无需选择工程即可一键配置 Keil TI 环境：自动检测路径，备份并更新 SDK 的 `syscfg.bat`、菜单导入配置以及当前用户的 Keil Tools 菜单
...
窗口顶部提供“工程转换”和“Keil TI 环境配置”两个独立入口，并记住上次使用的功能。
...
4. 选择一个空输出目录并开始转换；目标 IDE 存在时工具会自动执行目标工程构建验证，否则明确标记“未验证”。

### Keil TI 环境配置

1. 无需选择工程，进入“Keil TI 环境配置”。
2. 自动检测或手动选择 MSPM0 SDK、Keil 与带 `nw/nw.exe` 的图形化 SysConfig；Pack 仍由用户手动安装。CCS 随附的 CLI-only SysConfig 可用于构建，但不能配置 Keil Tools 图形化编辑入口。
3. 点击“一键配置 Keil TI 环境”，确认后更新 SDK 配置和当前用户的 Keil Tools 菜单。

SDK、Pack、CCS、Keil、SysConfig 路径、当前功能及工具目录搜索层级保存在本机 WebView 的 `localStorage`。CCS 会查找 `ccs-serverc.exe`，Keil 会通过注册表或目录查找 `UV4.exe`；搜索优先返回层级最浅的匹配项。
...
CCS → Keil 会遵循 `.cproject` 的 `sourceEntries/excluding`：CCS 工程树中带删除线、已排除构建的 C/C++、汇编和 SysConfig 文件不会进入 Keil 工程；头文件仍会复制并保留对应 Include Path，以复现 CCS 仍允许预处理器引用这些头文件的行为。这类标记不是 Windows 只读属性，转换器不会修改源文件属性。
...
- 修改前分别创建 `.ti-toolbox.bak` 备份；已存在备份时不覆盖。
...
```powershell
cargo test -p ti-toolbox-core --manifest-path src-tauri/Cargo.toml
```
