...
Keil 已安装 Pack 从 `ARM/PACK/TexasInstruments/<Pack>/<Version>` 自动识别。`.Web` 下的 PDSC 只作为在线目录和 `PackID` 元数据，不会显示成“已安装”；缺失时工具打开 `keil.arm.com` 官方页面，由用户自行下载和安装，不调用 Pack Installer。
